...
const RegisterOptions = ({ navigation, route }) => {
  ...
  const handleStoreFormChange = (name, text) => {
  ...
  const handleFarmFormChange = (name, text) => {
  ...
  const handleLogisticsFormChange = (name, text) => {
  ...
  const handleRegistration = () => {
    (selectedAccount === "buyerAccount" && handleBuyerRegistartion()) ||
      (selectedAccount === "storeAccount" && handleStoreRegistartion()) ||
      (selectedAccount === "farmersAccount" && handleFarmRegistartion()) ||
      (selectedAccount === "LogisticsAccount" && handleLogisticsRegistartion());
  };

  const handleBuyerRegistartion = async () => {
    try {
      setLoading(true);
      const sendData = await fetch(
        `https://farmyapp-smju.onrender.com/api/v1/user`,
        {
          method: "POST",
          headers: {
      ...
      dataRes && navigation.navigate("LoginOptions");
      console.log(dataRes);
    } catch (error) {
      console.log(error);
    }
  };
  const handleStoreRegistartion = async () => {
    try {
      setLoading(true);
      const sendData = await fetch(
        `https://farmyapp-smju.onrender.com/api/v1/store`,
        {
          method: "POST",
          headers: {
            "content-type": "application/json",
  ...
  const handleFarmRegistartion = async () => {
    try {
      setLoading(true);
      const sendData = await fetch(
  ...
  const handleLogisticsRegistartion = async () => {
    try {
      setLoading(true);
      const sendData = await fetch(
        `https://farmyapp-smju.onrender.com/api/v1/farm/`,
      ...
      <ScrollView>
        ...
        <View style={styles.categorySelector}>
          ...
          <View style={styles.loginMain}>
            <View style={styles.loginBox}>
              {(selectedAccount === "buyerAccount" && (
                ...
                (selectedAccount === "logisticsAccount" && (
                  <LogisticsForm
                    handleLogisticsFormChange={handleLogisticsFormChange}
          ...
          <View style={styles.otherCTA}>
            ...
            <TouchableOpacity
              onPress={() => navigation.navigate("LoginCategory")}
            >
